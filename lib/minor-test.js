...
var MinorTest = {
  ...
  _before : async function () {
    ...
    await this.start();
    // backwards compatible with 'done' callback
  ...
  _waitForEventQueue : async function () {
    const eventQueueWaitTimeout = this.options.eventQueueWaitTimeout || 1000
    if (process.env.TEST_DEBUG === '1') {
      console.log('waiting for browser event queue to empty');
    }
    let timedout = false;
    return new Promise((resolve) => {
      const timeoutHandle = setTimeout(() => {
        if (process.env.TEST_DEBUG === '1') {
          console.log('timed out while waiting for browser event queue to empty. Zombiejs browser dump=', this.browser.dump());
        }
        timedout = true;
        // turn off the event loop to prevent errors once browser is destroyed
        this.browser._eventLoop.active = null;
        resolve();
      }, eventQueueWaitTimeout);
      this.browser._wait(null, undefined).then(() => {
        if (timedout) { return }
        clearTimeout(timeoutHandle);
        if (process.env.TEST_DEBUG === '1') {
          console.log('browser event queue emptied');
        }
        resolve();
      })
    }).catch((e) => {
      if (process.env.TEST_DEBUG === '1') {
        console.log('error while waiting for browser event queue to empty', e);
      }
    })
  },
  ...
  _beforeEach : async function () {
    ...
    this.clear();
    // backwards compatible with 'done' callback
    if (this.beforeEach.length > 0) {
      return new Promise((resolve, reject) => {
        try {
          this.beforeEach(resolve);
        } catch (e) {
          reject(e);
        }
      })
